...
| Split temporal (train <2020…) | `stride` no `TextDataset` | Não há eixo temporal; a decisão de dados é sliding vs chunked (§6.2). |
| `train.py` vs `tune.py` (Optuna) | `train.py` com early stopping | Em pré-treino de LM não se faz HPO caro por trial; o loop já traz save-best + patience (§7.6). |
...
│   ├── tokenizer/bpe.py  # BPE byte-level (§3)
│   ├── data/
│   │   ├── scrape.py     # Wikipedia → JSONL (§6.1)
│   │   └── dataset.py    # TextDataset chunked (§6.2)
│   ├── model/
│   │   ├── layers/       # rmsnorm, swiglu, rope, attention (§5)
│   │   ├── block.py      # bloco pre-norm (§4.1)
│   │   └── transformer.py# modelo completo + weight tying + init (§4)
│   ├── training/
│   │   ├── train.py      # AdamW, scheduler, BF16, early stopping (§7)
│   │   └── evaluate.py   # loss, accuracy, BPC (§8.2)
│   └── inference/
│       └── generate.py   # KV-cache + sampling (§5.4)
...
## Roadmap (fora do escopo do pré-treino, seção 9 do paper)
